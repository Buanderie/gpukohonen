...
namespace GPUKohonenLib
{
    public class KohonenMap
    {
        ...
        public Bitmap GetBitmap()
        {
            ...
            for( int i = 0; i < m_Height; ++i )
                for( int j = 0; j < m_Width; ++j )
                    bm.SetPixel(j, i, Color.FromArgb(   (int)Math.Floor(Math.Min((polbak[0,m_Width*i+j]),255)), 
                                                        (int)Math.Floor(Math.Min((polbak[1,m_Width*i+j]),255)),
                                                        (int)Math.Floor(Math.Min((polbak[2, m_Width * i + j]), 255))
                                                     ));
            return bm;
        }
